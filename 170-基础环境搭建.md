...
线上地址演示`http://ihrm-java.itheima.net/#/login`

本地启动项目

![image-20211020084353930](assets/image-20211020084353930.png)

![image-20211020084300845](assets/image-20211020084300845.png)

1. HRSAAS - 人力资源管理系统 管理公司员工入职 调岗 审批 离职 考勤...
2. RBAC -  是基于角色的访问权限控制(Role-Based-Access-Control) - **不同的人登录到这个系统中看到的界面效果是不一样的**

![image-20211020085528590](assets/image-20211020085528590.png)
...
- jsonp get请求 基于script标签动态加载外部资源特性
- cors - 后端
- 反向代理 -前端
...
> `vue-element-admin` 有一个成熟的[集成方案](https://github.com/PanJiaChen/vue-element-admin)，里面包含了所有的业务功能和场景,并不适合直接拿来进行二次开发,
>
> 但是可以通过该项目中的一个案例来进行学习和使用

这里是[官网地址](https://panjiachen.gitee.io/vue-element-admin-site/zh/)

这里是[线上demo地址](https://panjiachen.github.io/vue-element-admin/#/dashboard),   [github地址](https://github.com/PanJiaChen/vue-element-admin) 

> 如果你想查看该项目的具体功能和效果，可以拉取代码，启动进行预览￥ 

```bash
$ git clone https://github.com/PanJiaChen/vue-element-admin.git  #拉取代码  github 会安装不上依赖 (找码云)
...
> 集成方案并不适合我们直接拿来进行二次开发，[基础模板](https://gitee.com/panjiachen/vue-admin-template)则是一个更好的选择
...
```bash
$ node -v #查看node版本 node版本12+  小于14.18
$ npm  -v #查看npm版本
```

![image-20210116233911391](assets/image-20210116233911391.png)
...
**注意: 由于 github 挂了,  github的包下载下来, npm i 会成功不了!!!**
...
或者 
$ git clone https://gitee.com/panjiachen/vue-admin-template.git　hrsaas　# 拉取基础模板的代码  (码云)
```
...
```bash
$ yarn dev #启动开发模式的服务  npm run dev
```
...
![image-20211020105302440](assets/image-20211020105302440.png)

### **main.js**

![image-20200824153141764](assets/image-20200824153141764.png)

删除 main.js **`mock数据`**的部分，删除**`mock`**文件夹，我们开发的时候用不到模拟数据 (有线上接口和本地)
...
> **`settings.js`**则是对于一些项目信息的配置，里面有三个属性 **`title`**(项目名称)，**`fixedHeader`**（固定头部），
>
